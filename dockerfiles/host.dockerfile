...
RUN mkdir /root/models/ && cd models/ && mkdir covid_press && cd ..
...
# RUN gsutil cp -r gs://mlops-project-338109-aiplatform//root/models/covid_press/ ./root/models/

EXPOSE 8080

ENTRYPOINT ["python", "-u", "src/api/setup_inference.py"]